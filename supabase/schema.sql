...
-- ─── Sale side (removed) ──────────────────────────────────────────────────────
-- The buyers / sales / sale_items / sale_payments tables and the app code that
-- used them were removed while the purchase side is being finalised. The tables
-- were NOT dropped in Supabase — they still exist, empty and unreferenced. When
-- the sale side comes back, recover the DDL and the pages from git history
-- (the commit that removed them) rather than rewriting from scratch.
...
-- ─── Row Level Security ───────────────────────────────────────────────────────
-- All tables are readable/writable only by a signed-in Supabase Auth
